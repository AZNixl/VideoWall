...
- 按文件夹浏览（`MediaStore` 的 `BUCKET_ID` 归组）
- 首页可切**单列列表**或**两列网格** —— 这一档**贯穿两级**：点进文件夹后的视频列表也是两列卡片
- **下拉刷新**重新扫描媒体库（不用重启应用）
- 顶栏第二个图标键开**浏览设置**弹窗：排布 + 文件夹排序 + 视频排序
  （名称 / 时长 / 大小 / 日期；顺序的措辞随依据变 —— `A → Z`、`最短优先`、`最新优先`……）
- 文件夹可以在设置里排除掉，首页不再出现
- 会记住上次选的那 4 个视频
...
- **单视频专属体验**（只播一路，或把某一路放大到全屏时自动启用）：
  ...
  多路播放时这些全部关闭 —— 四宫格里做全屏手势既没地方滑，也容易误触。
...
- **解码器不可指定**（见 §4）
- **字幕只支持外挂 `.srt`**：内嵌字幕轨（mov_text / 3GPP）的字节格式因封装而异，
  要按格式分别解析，本版没做。自动查找同名 `.srt` 只在文件已进媒体库、
  且当前权限看得到时才有效 —— Android 13+ 只授了 `READ_MEDIA_VIDEO` 时基本查不到，
  这时会弹系统文件选择器让你手动挑一个。编码按 UTF-8 试，失败转 GB18030（中文字幕常见 GBK）
- 没有「1 主 3 副」布局
