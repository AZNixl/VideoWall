...
| **首页三图标键** | 标题右侧：主题切换（亮/暗）、文件夹排布（列表 ⇄ 两列网格）、设置 |
| **网格排布贯穿两级** | 选了两列网格，首页文件夹是网格，**点进文件夹后的视频列表也是两列卡片** |
| 选好再播、铺满全屏 | 选择页 → 「开始播放」→ 进沉浸式，隐藏系统栏 |
| **首次启动介绍页** | 讲清用法与权限，拒绝授权后还有补救指引；设置里可重新查看 |
| **4K 起播前预检** | 有 4K / 多路 2K 时先给结论性提醒（可勾「不再提醒」），细节看格子上的信息标签 |
...
首次启动 → 介绍页（用法 4 条 + 权限说明 + 授权按钮）
  ...
  │    · 排布可切「单列列表」或「两列网格」，**这一档同时作用于文件夹内部**：
  │      首页是网格，点进去的视频列表也是两列卡片（缩略图 + 右上角勾选）
