...
## v3.2 — 音轨（多语言配音）选择、字幕定位修正、真机验证

v3.1 那批功能这次补上了真机验证，并修了验证时暴露的两个问题。

### 新增：单视频时的音轨 / 语言选择

测试片是「攻壳机动队 2160P BDRip 日英双语 FLAC」，有两条音频轨 —— 之前没法选。

- 顶栏第 8 个键（地球图标），只在单视频时出现
- 列出 `MediaPlayer.getTrackInfo()` 里的音频轨，语言用 ISO 639-2 码映射成中文
  （`jpn` → 日语、`eng` → 英语 ……），无语言标记的只显示编号
- 选中就 `selectTrack()` 切过去

**一个必须记住的坑**：`getTrackInfo()` 返回的是**所有类型**的轨道（视频 / 音频 / 字幕混在一个数组里），
下标是全局的。所以选的时候要把**原下标**交回 `selectTrack()`，
不能"音频轨里的第 2 条"直接当参数传 —— 那样会切到别的轨上去。

顶栏键也因此从 7 个涨到 8 个，152px 一个在 3168 宽的横屏上已经贴边，所以给键加了横向滚动容器兜底。

### 修：字幕位置不对（实测数据）

用户反馈"字幕位置不对"。截图 + `dumpsys` 量出来的实情是：

```
竖屏：格子 1432×3160，片源 3840×2160
      缩放按宽度算 → 画面只有 1432×805，居中 → y ≈ 1181..1986
      而字幕按"屏幕底边往上 96dp"放 → y ≈ 2734..2784
      → 字幕落在画面下方 750px 的黑边里，离画面老远
```

原来写的是"距屏幕底边 96dp"，但**竖屏看横屏片子时画面只占屏幕中间一条**，
贴着屏幕底边放必然掉进黑边。改成按**视频画面**的底边定位：

`VideoView.onMeasure` 在 EXACTLY 约束下会把自己缩到视频宽高比 ——
**它量出来的高度就是画面实际显示区域**，直接拿它算即可。
另外控制条显示时取两者更靠上的那个约束，免得压住按钮。

修复后实测（横屏）：画面底边 1432、字幕底边 1372 → 正好在画面底边上方 `d(16)`=64px ✓

### 改：字幕改成一个设置弹窗（开关 / 字号 / 编码）

原来"点一下循环开关"，但字号和编码也得能调，循环表达不了。改成弹窗：

| 组 | 内容 |
|---|---|
| 状态 | 已加载 N 条（来源）／还没加载 |
| 开关 | 开启 / 关闭 |
| 文本大小 | 小 / 标准 / 大 / 特大（13 / 16 / 20 / 24 sp） |
| 字符编码 | 自动 / UTF-8 / GB18030 / BIG5 / UTF-16 |

**编码切换为什么要缓存原始字节**：`loadSrt` 现在把文件的 `byte[]` 留在内存（`subRaw`），
改编码时直接用这份字节重新解码 —— 不用再读一次文件，也不怕那个 URI 已经失效。
自动档的判据是"UTF-8 解出来有没有 `U+FFFD` 替换字符"：GBK 中文喂给 UTF-8 解码器
几乎必然产生替换字符，反过来不会。够用的启发式。顺手剥掉 BOM，
否则第一条字幕会带个看不见的字符。
...
| 模式 | 行为 |
|---|---|
| **文件夹** | 原来的：文件夹列表 → 点进去是视频 |
| **树形目录** | 按真实路径层级展开，点目录行就地展开/收起 |
| **视频** | 不分文件夹，所有视频平铺（仍然受排布/排序影响） |

树形目录要路径数据，两个来源形状不一样，得统一：
API 29+ 用 `RELATIVE_PATH`（`"DCIM/Camera/"`），更老的用已弃用的 `DATA`（`"/storage/emulated/0/DCIM/Camera/x.mp4"`）。
`toRelDir()` 把两者都归成 `DCIM/Camera` —— 绝对路径要先砍掉文件名再砍存储前缀。
展开状态放内存（`treeExpanded`），不值得持久化。

### 修：弹窗里选完不即时生效，要重开才看到

**这条是用户实测报的**，而且症状很具体："选择后不是即时变动，要点完成，重进才会看到已经切到相应的按键"。

根因是**原地重建视图树**这条路不稳：原来是每次选择都 `box.removeAllViews()` 再重建。
普通界面没问题，但**对话框的窗口尺寸是 `show()` 的时候定下来的**，
重建之后要靠一次完整 relayout 才反映得出来 —— 实测就是"选完没变"。

改成 **`chipRow(RowState, OnPick)`：点完只对胶囊 `setTextColor` / `setBackground` / `setText`，
完全不增删 View**。只触发重绘，不依赖布局流程。排序那组的顺序键文字会变
（`A → Z` ↔ `最多优先`），所以连文字也走 `RowState` 供给。
浏览设置弹窗用的是同一套，一并改掉。

> 旁证：用户机器上的偏好里 `subSize=24` —— 说明他点过"特大"，**设置是生效了的**
> （字号确实变了），只是胶囊高亮没跟着走。正好对上这个根因。

---

### 真机验证结果（v3.1 + v3.2 的功能）

查证手段：`adb input tap/swipe` + `dumpsys activity top` 量 View 边界 + `logcat` 打点。
**注意：这台设备 `screencap` 出来的尺寸会随握持方向变（3168×1440 / 1440×3168），
`input tap` 用的是当前显示方向的坐标系 —— 方向搞错就会"点了没反应"。**
另外控件 4 秒自动隐藏，点顶栏键之前得先点一下画面把控件唤出来。

| 功能 | 证据 |
|---|---|
| 单击暂停 | `tap#pause cell=0` |
| 双击快进 10s | `tap#seek10 cell=0` |
| 右半屏上滑调音量 | `gesture volume -> 49/160 → 142/160` |
| 左半屏上滑调亮度 | `gesture brightness -> 44% → 100%` |
| A-B 循环 | 用户手动确认可用 |
| 音轨列表 | `音轨 2 条: [音轨 1 · 日语, 音轨 2]` |
| 音轨切换 | `切到音轨 2（音轨 2）` 无异常 |
| 字幕加载 | `已有 907 条，on=true`（自动找到同名 .srt） |
| 字幕位置 | 画面底边 1432 / 字幕底边 1372 = `d(16)` ✓ |
| 单视频三键显隐 | 8 个键全在、A-B/字幕/语言 alpha=0.5（未激活态）✓ |
| 树形目录 | `树形目录：37 个目录，顶层 6 个，根下视频 0` |

---

### 修：单路音频时选中框不随控件隐藏

`updateCellChrome` 里的高亮判断带了 `|| audioFocusMode` —— 本意是"单路音频时始终标出谁在出声"，
结果控件自动隐藏后那道强调框还留着，看着就是个没擦干净的残留。
改成只跟 `controlsVisible` 走：想看是哪一格出声，点一下唤出控件即可。

### 新增：首页下拉刷新

媒体库原本是启动时一次性缓存的，在别的应用里刚拍完视频、或刚把文件传进手机，
非得重启应用才看得到。包一层 `SwipeRefreshLayout`。
注意 `loadLibrary()` 有防重入闸门（`libraryLoadStarted`），刷新前得先放开，
否则第二次下拉什么都没发生。

### 新增：浏览设置弹窗（排布 + 排序）

顶栏那个"列表/网格"键改成开弹窗，里面三组：
...
选项划分和词汇参考 NextPlayer 的 `QuickSettingsDialog`：**顺序不写"升序/降序"，
而是按排序依据显示** `A → Z` / `Z → A`、`最短优先` / `最长优先`、`最小优先` / `最大优先`、
`最旧优先` / `最新优先` —— 对用户来说"最少优先"比"升序"好懂得多。

排序默认给的是**降序**（数量最多在前 / 日期最新在前），两个默认都合理。
另外给 `Item` 补了原始字节数 `sizeBytes`（`size` 是给人看的格式化串，排序不能用它）。

### 新增：单视频手势 + A-B 循环 + 字幕

只在**屏幕上只有一路视频**时启用（只选了 1 个，或把某一路放大到全屏）。
多路沿用原来的交互 —— 四宫格里做全屏手势既没地方滑，也容易误触。
...
- **左右按"滑动起点在哪一侧"判定**，不跟手指当前所在侧 —— 手指滑过中线不该中途换功能。
- **单击走 `onSingleTapConfirmed`**（要等 ~300ms 看是不是双击），而多路用 `onSingleTapUp`
  立刻响应 —— 多路等这 300ms 纯属拖慢手感，所以在同一个 GestureDetector 里分开处理。
- **亮度只改本窗口**（`WindowManager.LayoutParams.screenBrightness`），不动系统设置；
  退回选择页时还原，否则那个"调暗"会一直留在选择页上。
- **A-B 三态画在键上**（未设/只设了 A/循环中），不另外弹窗。B 点要求离 A 点至少 1 秒。
- **字幕自己解析 SRT，没用 MediaPlayer 的定时文本轨**：内嵌字幕轨（mov_text / 3GPP）
  的字节格式因封装而异，要按格式分别解析；而实际用得到的场景九成是"视频旁边放一个同名 .srt"。
  自己解析格式简单、完全可控，渲染也只是一个随播放位置更新的 TextView，不依赖播放器 API。
- **字幕编码试两遍**：UTF-8 解出来有替换字符就转 GB18030 ——
  网上的中文字幕有相当一部分是 GBK，一律按 UTF-8 读会得到满屏问号。
- 找同名字幕只查得到已进媒体库、且当前权限能看到的文件；Android 13+ 只授了
  `READ_MEDIA_VIDEO` 时基本查不到，这时走 SAF 让用户手动选一个 `.srt`。
  手动选的那个按视频 id 记在内存里（不落盘 —— 字幕只在本次播放用，没必要持久授权）。

### 改：GitHub 构建用英文名

- Release 资产名 `VideoWall-<tag>-release.apk`（原来是 `app-release.apk`，下几版之后分不清谁是谁）
- release notes 改英文
- 新增 `values-en/strings.xml`：英文环境下启动器显示 `VideoWall`，中文环境仍是「视频墙」

> **本版未做真机验证** —— 构建到一半手机断连了（`adb: no devices/emulators found`）。
> 只验证到「编译通过 + 出包」。手势、A-B、字幕这三块**没有在设备上跑过**，
> 已知的风险点是手势阈值和字幕在 Android 13+ 上的自动查找（多半查不到，会退回手动选文件）。
...
| 点击位置 | 修复前日志 |
|---|---|
| 左上格黑边 (360,80) | `tap#cell cell=0` ← 触发 |
| 左上格画面 (360,790) | **无任何日志** |
| 右上格画面 (1080,790) | **无任何日志** |
| 左下格画面 (360,2350) | **无任何日志** |

**关键在于"连 cell 的兜底都没触发"** —— 如果只是「VideoView 把触摸吞了但没响应」，
事件会照常继续往下落到 cell（FrameLayout 的分派循环在子 View 返回 false 时会接着试下一个），
cell 的监听器就该响。可它没响，说明**画面那块区域上的触摸在视图层级里压根没落到可用目标上**。

`VideoView` 是 `SurfaceView`，它的 Surface 由系统合成器单独摆放、并在窗口上开洞，
不同 ROM 在触摸分派上的表现不一致。与其继续猜，不如换做法：

**修法**：在 `VideoView` 之上盖一层**没有 Surface 的普通 View**（`Cell.tapCatcher`）来收点击。
普通 View 不参与 surface 合成，行为完全可预期。
它加在 VideoView 之后、`bottomOverlay` 之前 —— 逐格控制条与进度条仍在它上面，按钮和拖动不受影响。

**验证**：同样四个点重测，全部命中 `tap#catcher` ✓

三条通路都留了 `Log.d` 标记（`tap#video` / `tap#cell` / `tap#catcher`），
下次再报"点不灵敏"，`adb logcat -s VideoWall` 一眼看出是哪条走到了。
